...
angular.module('esn.ui', [
  ...
  .directive('fab', function(FAB_ICONS, DEFAULT_COLOR_CLASS) {
    return {
      ...
      replace: true,
      scope: true,
      template: require("../../views/modules/ui/fab.pug"),
  ...
  .directive('dynamicFabDropup', function() {
    return {
      restrict: 'E',
      template: require("../../views/modules/ui/dynamic-fab-dropup.pug"),
      scope: {
        anchor: '@'
      },
      link: function($scope, element) {
    ...
    return {
      ...
      link: function(scope, element, attrs) {
        ...
        function _toggle(toggleTransistion) {
          ...
          element.parent().find('ul:not(".not-toggled")').stop(true, false).slideToggle(toggleTransistion);
        }
      }
    };
  });
